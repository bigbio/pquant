...
quantdata2proteus <- function(df){
  ...
  df['Reverse'] <- NA
  df['Contaminant'] <- NA
  output_path <- 'proteus_formatted.tsv'
...
# extract DE results from output generated by either Proteus or MSstats
getDifferentialExpressionResults <- function(object,
                                             method="proteus",
                                             cond_label=NULL){
  if(!method %in% c("msstats", "proteus")) {
    stop("Input method must be one of 'proteus' or 'msstats'...")
  }
  out_cols <- c("protein", "logFC", "p_value", "p_value_adj")
  if(method=="msstats"){
    if(class(object)!="list") stop("Expect an object of type list
                                   as imput with method msstats")
    df_de <- object[["ComparisonResult"]]
    df_de <- subset.data.frame(df_de,
                               select=c("Protein","log2FC","pvalue","adj.pvalue"))
  }
  if(method=="proteus"){
    if(class(object)!="data.frame") stop("Expect an object of type data.frame
                                         as imput with method proteus")
    df_de <- subset.data.frame(object,
                               select=c("protein","logFC","P.Value","adj.P.Val"))
  }
  names(df_de) <- out_cols
  df_de$method <- method
  return(df_de)
}
