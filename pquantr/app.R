...
server <- function(input, output, session) {
    ...
    output$contents <- renderDT({
        if(is.null(dataControl$csvFile_state)){
            ...
            datatable(fileData <- read.csv(input$csvFile$datapath),
    ...
    observeEvent(input$submit_anno, {
      if(input$start_preprocess == 0 & input$rdataFile_submit == 0){
          sendSweetAlert(
              session = session,
              title = "Fail",
              text = "Please preprocess '.csv' file or  upload '.RData' file first",
              type = "error",
              closeOnClickOutside = TRUE,
              width = 500)
      } else{
          ...
          tabMelt$seqsam <- paste0(tabMelt$sequence, "shiny-app", tabMelt$sample)
          tabMelt <- tabMelt[order(tabMelt$seqsam),]
          s2s <- setNames(tabMelt$sequence, tabMelt$seqsam)
          r <- rle(tabMelt$seqsam)
          tabMelt$uniseq <- paste0(rep(s2s[r$values], times=r$lengths), "shiny-app", unlist(lapply(r$lengths, seq_len)))
